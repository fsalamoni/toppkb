...
export function MuscleHint({ exerciseId, variant = 'inline', className = '' }: MuscleHintProps) {
  ...
  if (variant === 'badge') {
    return (
      <div className={`inline-flex items-center gap-1 text-xs text-blue-400 ${className}`}>
  ...
  if (variant === 'card') {
    return (
      <div className={`space-y-2 rounded-lg border border-blue-500/30 bg-blue-500/5 p-3 ${className}`}>
        {topMucs.length > 0 && (
          <div>
